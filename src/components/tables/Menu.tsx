...
export default function Menu() {
  // State cho danh mục đã xóa mềm (fix lỗi thiếu biến)
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (res.data && typeof res.data === 'object' && res.data !== null) {
        const data = (res.data as { data?: IProduct[]; last_page?: number });
        if (Array.isArray(data.data)) {
          products = data.data;
        }
        if (typeof data.last_page === 'number') {
      ...
      setProducts(products.map((item: IProduct) => {
        ...
        // Ensure all IProduct properties are present
        return {
          ...item,
          id: item.id,
          id_category: (item as IProduct).id_category ?? (item as IProduct).id_category,
          id_user: (item as IProduct).id_user,
          name: (item as IProduct).name,
          slug: (item as IProduct).slug,
          price: (item as IProduct).price,
          status: item.status,
          image: (item as IProduct).image,
          meta_description: (item as IProduct).meta_description,
          detail_description: (item as IProduct).detail_description,
          quantity_sold: (item as IProduct).quantity_sold,
          created_at: (item as IProduct).created_at,
          updated_at: (item as IProduct).updated_at,
          deleted_at: (item as IProduct).deleted_at,
          is_active,
        };
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModalState({
        open: true,
        title: "Lưu thất bại!",
        description: errorMessage || "Có lỗi xảy ra khi lưu sản phẩm.",
        emoji: <span style={{ fontSize: 28 }}>❌</span>,
        acceptText: "OK",
        onAccept: () => setModalState((prev) => ({ ...prev, open: false })),
      });
